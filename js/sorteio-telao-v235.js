...
let quadroAjusteNome = null;
let ultimoNomeGanhador = '';
...
function iniciarConfetes() {
    ...
    function desenhar(agora) {
        const intervalo = ultimoQuadro === null ? 1000 / 60 : Math.min(40, Math.max(0, agora - ultimoQuadro));
        const fatorQuadro = intervalo / (1000 / 60);
        ultimoQuadro = agora;
        contexto.clearRect(0, 0, innerWidth, innerHeight);
        particulas.forEach(p => {
            const amortecimento = Math.pow(.992, fatorQuadro);
            p.x += p.vx * ((1 - amortecimento) / (1 - .992));
            p.y += (p.vy * fatorQuadro) + (.5 * p.gravidade * fatorQuadro * fatorQuadro);
            p.vy += p.gravidade * fatorQuadro;
            p.vx *= amortecimento;
            p.rotacao += p.giro * fatorQuadro;
            p.vida -= fatorQuadro;
            contexto.save();
    ...
    quadroConfetes = requestAnimationFrame(desenhar);
}

function ajustarEscalaNomeGanhador(nome) {
    const nomeSeguro = String(nome || '—').trim() || '—';
    ultimoNomeGanhador = nomeSeguro;
    nomeGanhador.textContent = nomeSeguro;
    nomeGanhador.classList.toggle('nome-com-palavra-longa', nomeSeguro.split(/\s+/).some(parte => parte.length > 22));

    if (quadroAjusteNome) cancelAnimationFrame(quadroAjusteNome);
    quadroAjusteNome = requestAnimationFrame(() => {
        const quantidadeCaracteres = Array.from(nomeSeguro).length;
        const quantidadePalavras = nomeSeguro.split(/\s+/).filter(Boolean).length;
        const tamanhoBase = Math.min(148, Math.max(58, innerWidth * .088));
        const fatorCaracteres = quantidadeCaracteres <= 22 ? 1 : Math.max(.48, Math.sqrt(22 / quantidadeCaracteres));
        const fatorPalavras = quantidadePalavras <= 3 ? 1 : Math.max(.58, 1 - ((quantidadePalavras - 3) * .09));
        const tamanhoMinimo = Math.min(48, Math.max(34, innerWidth * .032));
        let tamanho = Math.max(tamanhoMinimo, tamanhoBase * Math.min(fatorCaracteres, fatorPalavras));

        nomeGanhador.style.fontSize = `${tamanho}px`;
        let tentativas = 0;
        while (
            tentativas < 36
            && tamanho > tamanhoMinimo
            && (nomeGanhador.scrollHeight > (tamanho * 2.05) || nomeGanhador.scrollWidth > (nomeGanhador.clientWidth + 2))
        ) {
            tamanho = Math.max(tamanhoMinimo, tamanho - 2);
            nomeGanhador.style.fontSize = `${tamanho}px`;
            tentativas += 1;
        }
    });
...
function iniciarSorteio(dados) {
    ...
    timers.push(setTimeout(() => {
        ...
        ajustarEscalaNomeGanhador(dados.ganhador);
        iniciarConfetes();
...
window.addEventListener('resize', () => {
    ajustarCanvas();
    if (ultimoNomeGanhador && !estadoResultado.hidden) ajustarEscalaNomeGanhador(ultimoNomeGanhador);
});
